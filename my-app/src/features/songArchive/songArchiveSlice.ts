...
function rangeArchive(from: number, to: number): object {
  return {title: `${from}〜${to}`, songs: range(from, to)}
}
...
export const songArchives: SongArchive[] = [
  ...
  // 本番用
  {title: '5色 青', songs: [3, 5, 6, 12, 14, 24, 30, 31, 50, 57, 61, 62, 69, 70, 74, 75, 76, 82, 91, 100]},
  {title: '5色 桃', songs: [1, 4, 13, 16, 22, 28, 34, 40, 48, 51, 58, 65, 66, 72, 73, 80, 83, 84, 86, 97,]},
  {title: '5色 黄', songs: [2, 7, 10, 18, 32, 33, 37, 39, 46, 47, 55, 60, 78, 79, 81, 85, 87, 89, 94, 96,]},
  {title: '5色 緑', songs: [8, 9, 11, 15, 17, 20, 23, 26, 29, 35, 36, 38, 41, 42, 54, 59, 68, 71, 92, 93,]},
  {title: '5色 橙', songs: [19, 21, 25, 27, 43, 44, 45, 49, 52, 53, 56, 63, 64, 67, 77, 88, 90, 95, 98, 99,]},
  // @ts-ignore
  // rangeArchive(1, 20),
  ...
  [...Array(5).keys()]
    .map(i => i * 20 + 1)
    .map(i => rangeArchive(i, i + 19))
).concat(
  // @ts-ignore
  [...Array(10).keys()]
    .map(i => i * 10 + 1)
    .map(i => rangeArchive(i, i + 9))
)
